...
for ii in [0, 1]:
  ...
  KK = 2
  mesh = ax[ii].contourf(ff.y[:, :, 12].T / 1e3, ff.x[:, :, 12].T / 1e3, ff.u_field[:, :, 12].reshape(RESU ** 2).reshape(RESU, RESU).T, 100, vmin=0, vmax=8)
  ax[ii].set_title('')
  ax[ii].set_ylim(-.4, .4)
  ax[ii].set_xlim(-.6, .8)
  mesh.set_clim(0, 8)
...
ax_cbar = fig.add_axes([p0[0] - 0.05, 2 * p0[1] - .1, p0[2], 0.05])
...
ax[1].set_ylabel('Northern Position (km)')
ax[0].xaxis.grid()
ax[1].xaxis.grid()
